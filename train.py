...
# According to the paper: We simply set feature matrix X = I as an identity matrix which means every word or document
...
def evaluate(features, labels, mask):
    ...
    with torch.no_grad():
        ...
        tm_mask = torch.unsqueeze(t_mask, 1).repeat(1, labels.shape[1])
